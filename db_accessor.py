...
def insert_entry(firstname, lastname, url_image, room_id):
    conn = get_connection()
    cur = conn.cursor()
    
    # Insert the new entry into the 'entries' table
    cur.execute('''
    INSERT INTO entries (firstname, lastname, url_image, room_id)
    VALUES (?, ?, ?, ?);
    ''', (firstname, lastname, url_image, room_id))
...
def get_entries_by_room(room_id):
    ...
    cur.execute('''
    SELECT firstname, lastname, url_image
    FROM entries
    WHERE room_id = ?;
    ''', (room_id,))
...
# Input is password, output is the room_id
def get_room_password(room_id):
    conn = get_connection()
    cur = conn.cursor() 
    cur.execute("SELECT room_password FROM rooms where room_id = ?" , (room_id,))
    row = cur.fetchone()
    conn.close()
    
    return row[0] if row else None

#Input room_id return password
